...
from Bert import BertSentimentModel
...
def main():
    ...
    amazontrain_data, amazontest_data = train_test_split(amazon_data, test_size=0.2, random_state=42)
    ...
    for batch_size in args.batch_sizes:
        for learning_rate in args.learning_rates:
            train_dataloader = movietokenize(train_data, batch_size)
            dev_dataloader = movietokenize(dev_data, batch_size)
            model = BertSentimentModel(freeze_bert=True)
    ...
    test_dataloader = movietokenize(test_data, best_batch_size)
    ...
    amazontest_dataloader = tokenize(amazontest_data, best_batch_size)
    amazontest_accuracy, amazontest_labels, amazontest_predictions = evaluate_model(best_model, amazontest_dataloader)
    print(f"Model accuracy on Amazon test data: {amazontest_accuracy:.4f}")
